...
import {
  X, Plus, Maximize2, Minimize2, GripVertical, Globe, RefreshCw,
  ArrowLeft, ArrowRight, ExternalLink, Monitor, Smartphone, Tablet,
  Copy, Layers, LayoutGrid, LayoutTemplate, Save, FolderOpen,
  ZoomIn, ZoomOut, Expand, Shrink, ChevronDown, Columns, Code
} from "lucide-react";
...
const DEVICE_PRESETS: Record<string, { label: string; width: number; height: number; icon: React.ReactNode }> = {
  desktop: { label: "Desktop", width: 1280, height: 800, icon: <Monitor size={13} /> },
  tablet: { label: "iPad", width: 768, height: 1024, icon: <Tablet size={13} /> },
  mobile: { label: "iPhone", width: 390, height: 844, icon: <Smartphone size={13} /> },
  custom: { label: "Custom", width: 480, height: 360, icon: <Expand size={13} /> },
};
...
function loadWorkspaces(): Workspace[] {
  try { return JSON.parse(localStorage.getItem(SAVED_WORKSPACES_KEY) || "[]"); }
  catch { return []; }
}
...
function PanelFrame({ panel, onUpdate, onRemove, onFocus }: {
  panel: Panel;
  ...
  const resizeRef = useRef<{ dir: string; startX: number; startY: number; w: number; h: number; px: number; py: number } | null>(null);
  ...
  const navigateTo = useCallback((url: string) => {
    ...
    const newHistory = [...panel.history.slice(0, panel.historyIdx + 1), clean];
    onUpdate(panel.id, {
      currentUrl: clean,
  ...
  const startDrag = (e: React.MouseEvent) => {
    ...
    const move = (e: MouseEvent) => {
      ...
      onUpdate(panel.id, {
        x: Math.max(0, dragRef.current.panelX + e.clientX - dragRef.current.startX),
        y: Math.max(0, dragRef.current.panelY + e.clientY - dragRef.current.startY),
      });
    };
  ...
  const startResize = (e: React.MouseEvent, dir: string) => {
    ...
    e.stopPropagation();
    onFocus(panel.id);
    resizeRef.current = { dir, startX: e.clientX, startY: e.clientY, w: panel.width, h: panel.height, px: panel.x, py: panel.y };
    ...
    const move = (e: MouseEvent) => {
      ...
      if (dir.includes("w")) { patch.width = Math.max(320, w - dx); patch.x = px + dx; }
      if (dir.includes("n")) { patch.height = Math.max(200, h - dy); patch.y = py + dy; }
  ...
  const copyUrl = () => navigator.clipboard.writeText(panel.currentUrl).catch(() => {});
  const openExternal = () => window.open(panel.currentUrl, "_blank");

  const innerW = panel.deviceMode === "custom" ? panel.width - 2 : device.width;
  const innerH = panel.deviceMode === "custom" ? (panel.height - 80) : device.height;
  const scale = panel.deviceMode !== "custom"
    ? Math.min((panel.width - 2) / device.width, (panel.height - 80) / device.height, 1) * (panel.zoom / 100)
    : panel.zoom / 100;

  const RESIZE_HANDLES = [
    { dir: "e", cls: "absolute right-0 top-1/2 -translate-y-1/2 w-2 h-12 cursor-ew-resize hover:bg-white/30 rounded-l-none rounded-r" },
    { dir: "w", cls: "absolute left-0 top-1/2 -translate-y-1/2 w-2 h-12 cursor-ew-resize hover:bg-white/30 rounded-r-none rounded-l" },
    { dir: "s", cls: "absolute bottom-0 left-1/2 -translate-x-1/2 h-2 w-12 cursor-ns-resize hover:bg-white/30 rounded-t-none rounded-b" },
    { dir: "n", cls: "absolute top-0 left-1/2 -translate-x-1/2 h-2 w-12 cursor-ns-resize hover:bg-white/30 rounded-b-none rounded-t" },
    { dir: "se", cls: "absolute bottom-0 right-0 w-4 h-4 cursor-se-resize hover:bg-white/30 rounded" },
    { dir: "sw", cls: "absolute bottom-0 left-0 w-4 h-4 cursor-sw-resize hover:bg-white/30 rounded" },
    { dir: "ne", cls: "absolute top-0 right-0 w-4 h-4 cursor-ne-resize hover:bg-white/30 rounded" },
    { dir: "nw", cls: "absolute top-0 left-0 w-4 h-4 cursor-nw-resize hover:bg-white/30 rounded" },
  ];
  ...
  return (
    <motion.div
      ...
      style={{ left: panel.x, top: panel.y, width: panel.width, zIndex: panel.zIndex, position: "absolute" }}
    ...
    >
      ...
      {RESIZE_HANDLES.map(h => (
        <div key={h.dir} className={h.cls} onMouseDown={e => startResize(e, h.dir)} />
      ))}
      ...
      <div className="glass-card rounded-xl overflow-hidden shadow-2xl border border-white/10"
        ...
        >
          ...
          {/* Nav */}
          <button onClick={goBack} disabled={panel.historyIdx <= 0} className="text-white/40 hover:text-white disabled:opacity-20 transition-colors p-0.5" onMouseDown={e => e.stopPropagation()}>
            <ArrowLeft size={12} />
          </button>
          <button onClick={goForward} disabled={panel.historyIdx >= panel.history.length - 1} className="text-white/40 hover:text-white disabled:opacity-20 transition-colors p-0.5" onMouseDown={e => e.stopPropagation()}>
            <ArrowRight size={12} />
          </button>
          <button onClick={() => onUpdate(panel.id, { currentUrl: panel.currentUrl })} className="text-white/40 hover:text-white transition-colors p-0.5" onMouseDown={e => e.stopPropagation()}>
            <RefreshCw size={12} />
          </button>
          ...
          <div className="relative" onMouseDown={e => e.stopPropagation()}>
            <button
              onClick={() => { setShowDevices(!showDevices); setShowZoom(false); }}
            ...
            {showDevices && (
              <div className="absolute top-6 right-0 glass rounded-lg shadow-xl z-50 py-1 min-w-[110px]">
                {Object.entries(DEVICE_PRESETS).map(([key, preset]) => (
                  <button
                    key={key}
                    onClick={() => { onUpdate(panel.id, { deviceMode: key as Panel["deviceMode"] }); setShowDevices(false); }}
          ...
          <div className="relative" onMouseDown={e => e.stopPropagation()}>
            <button
              onClick={() => { setShowZoom(!showZoom); setShowDevices(false); }}
            ...
            {showZoom && (
              <div className="absolute top-6 right-0 glass rounded-lg shadow-xl z-50 p-2 min-w-[120px]">
                <input
                  type="range" min={25} max={200} step={5} value={panel.zoom}
                  onChange={e => onUpdate(panel.id, { zoom: Number(e.target.value) })}
                  className="w-full accent-primary"
                />
                <div className="flex justify-between mt-1">
                  {[50, 75, 100, 150].map(z => (
                    <button key={z} onClick={() => onUpdate(panel.id, { zoom: z })}
          ...
          <button onClick={copyUrl} className="text-white/40 hover:text-white transition-colors p-0.5" title="Copy URL" onMouseDown={e => e.stopPropagation()}>
            <Copy size={12} />
          </button>
          <button onClick={openExternal} className="text-white/40 hover:text-white transition-colors p-0.5" title="Open in new tab" onMouseDown={e => e.stopPropagation()}>
        ...
        {/* Content */}
        {!panel.minimized && (
          <div className="relative overflow-hidden bg-white" style={{ height: panel.height - 40 }}>
            ...
            <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-2 py-0.5 bg-black/60 text-white/40 text-[10px]">
              <span>{new URL(panel.currentUrl.startsWith("http") ? panel.currentUrl : "https://" + panel.currentUrl).hostname}</span>
              <span className="flex items-center gap-2">
                <span>{panel.width}×{panel.height}</span>
                {panel.deviceMode !== "custom" && <span className="text-primary">{device.label} ({device.width}×{device.height})</span>}
...
function AddPanelModal({ onAdd, onClose }: { onAdd: (url: string, title: string) => void; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <form onSubmit={e => { e.preventDefault(); if (url) onAdd(url, title); }} className="space-y-3">
          <input
            value={url}
          ...
          <div className="grid grid-cols-2 gap-2">
            {QUICK.map(q => (
              <button
                key={q.title}
                type="button"
                onClick={() => { setUrl(q.url); setTitle(q.title); }}
          ...
          <div className="flex gap-2 pt-1">
            <button type="button" onClick={onClose} className="flex-1 py-2 rounded-lg border border-white/20 text-white/70 text-sm">Cancel</button>
            <button type="submit" className="flex-1 py-2 rounded-lg bg-primary text-white text-sm">Open Panel</button>
          </div>
        </form>
      </motion.div>
    </motion.div>
...
export function MultiTaskPanel() {
  ...
  const focusPanel = useCallback((id: string) => {
    topZ += 1;
    setPanels(p => p.map(panel => panel.id === id ? { ...panel, zIndex: topZ } : panel));
  }, []);
  ...
  return (
    <>
      {/* Panel layer */}
      <div className="fixed inset-0 z-30 pointer-events-none" data-testid="panel-layer">
        <AnimatePresence>
          {panels.map(panel => (
            <PanelFrame
              key={panel.id}
        ...
        <div className="relative">
          ...
          <AnimatePresence>
            {showWorkspaces && (
              ...
              >
                <p className="text-white/50 text-[10px] uppercase tracking-wider mb-2">Save Current</p>
                <div className="flex gap-1.5 mb-3">
                  ...
                  />
                  <button onClick={saveWorkspace} className="bg-primary text-white text-xs px-2 py-1 rounded" title="Save">
                    <Save size={12} />
                  </button>
                </div>
                {workspaces.length > 0 && (
                  <>
                    <p className="text-white/50 text-[10px] uppercase tracking-wider mb-1.5">Saved</p>
                    {workspaces.map(ws => (
                      <div key={ws.name} className="flex items-center gap-1 mb-1">
                        <button
                          onClick={() => loadWorkspace(ws)}
                          className="flex-1 flex items-center gap-1.5 text-white/80 hover:text-white text-xs text-left py-1"
                        >
                          <FolderOpen size={11} /> {ws.name} <span className="text-white/30">({ws.panels.length}p)</span>
                        </button>
                        <button onClick={() => deleteWorkspace(ws.name)} className="text-white/30 hover:text-red-400 transition-colors">
                          <X size={11} />
                        </button>
                      </div>
                    ))}
      ...
      <AnimatePresence>
        {showAddModal && <AddPanelModal onAdd={addPanel} onClose={() => setShowAddModal(false)} />}
      </AnimatePresence>
    </>
  );
}
